...
from colormatcher.cvdsimulator import make_cvd_transform, SEVERITY_MAP, DEFICIENCY_NAMES

ROOT = Path(__file__).resolve().parent


def create_app(database=DATABASE, *, catalog_directory=CATALOG_DIR, detect_shades=None):
    """Serve the guided flow; detect_shades optionally replaces the CV extractor."""
    ...
    def check_request():
        if request.method == "POST":
            ...
            if request.path != "/api/detect-shades" and (request.content_length or 0) > 100_000:
    ...
    def recommend():
        ...
        profile = get_cvd_profile(payload.get("user_id"), database)
        transform = None
        cvd = {"applied": False, "message": "No color vision profile saved. Matching uses original colors."}
        if profile is not None:
            if profile["type"] not in DEFICIENCY_NAMES:
                cvd["message"] = "Your color vision type is not supported yet. Matching uses original colors."
            else:
                try:
                    transform = make_cvd_transform(profile["type"], profile["severity_level"])
                    cvd = {"applied": True, "type": profile["type"],
                           "severity": SEVERITY_MAP[profile["severity_level"]],
                           "message": "Personal matches use your color vision simulation; aesthetic matches use original colors. Severity is a prototype preset."}
                    if profile["type"] == "tritan":
                        cvd["message"] += " Tritan simulation is especially approximate."
                except ImportError:
                    cvd["message"] = "Color vision simulation is unavailable: install requirements-cvd.txt. Matching uses original colors."
                except Exception:
                    app.logger.exception("Could not prepare CVD simulation")
                    cvd["message"] = "Color vision simulation is unavailable. Matching uses original colors."
        with SQLiteStorage(database) as storage:
            service = PreferenceService(storage)
            ...
            try:
                ...
                if transform is None:
                    raise
                app.logger.exception("CVD matching failed; using original colors")
                result = service.recommend(payload.get("user_id"), category, products, **options)
                cvd = {"applied": False, "message": "Color vision simulation failed. Matching uses original colors."}
        if result["personal_weight"] == 0:
            cvd = {"applied": False, "message": "100% aesthetic: matching uses original colors only."}
        result["cvd"] = cvd
