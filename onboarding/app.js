...
function stage(name) { for (const key of ['personal', 'aesthetic', 'cvd', 'balance']) { $(key + '-stage').hidden = key !== name; if (key === name) $('step-' + key).setAttribute('aria-current', 'step'); else $('step-' + key).removeAttribute('aria-current'); } clearResults(); $(name + '-stage').scrollIntoView({ behavior: 'smooth', block: 'start' }); }
...
async function loadFiles(kind, files) {
  ...
  try {
    ...
    } status(errors.length ? errors.join(' ') : 'References added. Select the makeup region in each photo.');
  } finally { loading = false; $(kind + '-upload').value = ''; updateButtons(); }
}
...
$('aesthetic-next').onclick = async () => { try { if ($('add-aesthetic').value === 'yes') { await saveSet('aesthetic'); $('weight').value = 60; $('weight').disabled = false; } else { $('weight').value = 100; $('weight').disabled = true; } $('balance-description').textContent = hasAesthetic() ? `Choose how much your taste and ${$('environment').value.trim()} (your estimate) should influence the result.` : 'No second reference set selected. Recommendations use 100% your own taste.'; weightLabel(); stage('cvd'); status('Enter the type and severity stated in your diagnosis.'); } catch (e) { status(e.message + ' Continue again to retry.'); } };
...
$('weight').oninput = weightLabel; $('balance-back').onclick = () => stage('cvd');
$('recommend').onclick = async () => {
  const ticket = ++rankingRequest;
  try {
    $('recommend').disabled = true; const data = await api('/api/recommend', { user_id: user(), category: $('category').value, mode: 'weighted', personal_weight: hasAesthetic() ? Number($('weight').value) / 100 : 1, ...(hasAesthetic() ? { environment_profile_id: confirmedEnvironment } : {}) }); if (ticket !== rankingRequest) return;
